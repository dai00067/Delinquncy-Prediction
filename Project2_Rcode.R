...
folder="C:/Users/Dichang/Desktop/Learning Cache/Freddie's/data2/"
...
modfile_2009Q1 <- read_modfile("C:/Users/Dichang/Desktop/Learning Cache/Freddie's/data2/historical_data1_Q12009.txt", "2009Q1")
modfile_2009Q2 <- read_modfile("C:/Users/Dichang/Desktop/Learning Cache/Freddie's/data2/historical_data1_Q22009.txt", "2009Q2")
modfile_2009Q3 <- read_modfile("C:/Users/Dichang/Desktop/Learning Cache/Freddie's/data2/historical_data1_Q32009.txt", "2009Q3")
modfile_2009Q4 <- read_modfile("C:/Users/Dichang/Desktop/Learning Cache/Freddie's/data2/historical_data1_Q42009.txt", "2009Q4")
...
file2 <- model_file[which(model_file$orig_upb < 1000000), c("fico", "loan_purpose", "orig_upb")]
...
explore_num <- function(xvar, xlabel){
  ...
  check_bad <- aggregate(bad ~ xbin, samp_mod, mean)
  
  ggplot(check_bad, aes(x=xbin, y=bad)) + 
    geom_bar(stat = "identity", color='blue', fill='blue') +scale_color_manual(values =c('blue')) +
    labs(title="Bad Rate") + ylab("Bad Rate") +xlab(xlabel)
...
mylogit <- glm(bad ~ fico+dti+ltv+orig_upb_rescaled+orig_upb_log+prepay_penalty+super_conforming+mi_flag, 
               data=samp_mod, family = "binomial")
mylogit
summary(mylogit)

# rerun logistic regression by retaining useful variables only
mylogit <- glm(bad ~ fico+ltv+mi_flag, 
               data=samp_mod, family = "binomial")
mylogit
summary(mylogit)
...
check_score <- function(dataset) {
  ...
  check_bad <- dataset %>%
    select(bad, xbin, predicted_score) %>%
    group_by(xbin) %>%
    summarize(predicted_bad=mean(predicted_score), actual_bad=mean(bad), score_label=median(predicted_score))
  as.data.frame
  check_bad
  
  ggplot(aes(x=1:nrow(check_bad), y = value, color = variable), data = check_bad) +  
    geom_line(aes(x=1:nrow(check_bad), y=predicted_bad, color="blue")) +
    geom_line(aes(x=1:nrow(check_bad), y=actual_bad, color="red")) +
    xlab("Score Decile") + ylab("Actual vs. Predicted") + labs("Actual Bad vs. Predicted Bad")
}
